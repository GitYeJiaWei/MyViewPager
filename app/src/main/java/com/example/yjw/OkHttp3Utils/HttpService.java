...
public class HttpService {
    ...
    public void getAsyncHttp(String url)
    {
        ...
        Request.Builder requestBuilder = new Request.Builder().url(HttpConfig.HttpHeader.concat(url));
    ...
    public void postAsyncHttp(String url, PostParameter parameter) {
        if (url.startsWith("/")) {    //判断首字符串是否相同
            url = url.substring(1);   //截断第一的字符
        }
        OkHttpClient okHttpClient = new OkHttpClient();
        ...
        Request request = new Request.Builder()
                .url(HttpConfig.HttpHeader.concat(url))
                .post(formBody)
                .build();
